...
We will also investigate an alternative synchronization protocol by making heavier usage of collected meta data.
...
\begin{verbatim}
Read all commits since the last synced commit from Source and write them to Target.
Let the Target compute the common ancestor of Target's and Source's head.
Read all changed data since the common ancestor from Source and write to Target.

if common ancestor == Source's head
  // Source has not changed since last sync, Target is ahead of Source.
  // Do nothing.
else if common ancestor == Target's head
  // Target has not changed since last sync. Source is ahead of Target.
  Fast-forward Target's head to Source's head.
else
  // Both Source and Target have changed data since last sync.
  Run three-way merge of common ancestor, Source's head and Target's head.
  Commit result as new head to Target.
\end{verbatim}
